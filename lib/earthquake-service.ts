...
export async function getRecentEarthquakes(
  ...
  try {
    console.log(
      `Fetching earthquakes from: ${USGS_API_BASE_URL}/query?${queryParams.toString()}`
    );
    const response = await fetch(
      `${USGS_API_BASE_URL}/query?${queryParams.toString()}`,
      {
    ...
    if (params.country && params.country !== "all") {
      earthquakes = earthquakes.filter((eq) =>
        eq.place?.toLowerCase().includes(params.country!.toLowerCase())
      );
    }
...
export async function getEarthquakeById(id: string): Promise<Earthquake> {
  ...
  try {
    ...
    try {
      const response = await fetch(
        `${USGS_API_BASE_URL}/query?format=geojson&eventid=${id}`,
        {
          cache: "no-store", // Disable caching to get fresh data
          next: { revalidate: 0 }, // Force revalidation on each request
        }
    ...
    try {
      ...
      const recentEarthquakes = await getRecentEarthquakes({
        ...
        limit: 2000, // Increase limit to find more earthquakes
        starttime: new Date(
      ...
      try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            console.error(
              `Failed to parse JSON from endpoint ${endpoint}:`,
              jsonError
            );
...
export async function getEarthquakeStatistics(
  ...
  try {
    ...
    const yearEarthquakes = await getNearbyEarthquakes(
      latitude,
      longitude,
      radius,
      days,
